...
if(isset($_POST['UD_submit'])){
    ...
    $phone = $_SESSION['teacher_mob'];

  $query = "UPDATE  `teacher_details` SET `name`='$name', `email`= '$email', `password`= '$password' where phone = $phone";
    $result1 = mysqli_query($conn,$query);
}

//   print_r($_SESSION['teacher_mob']);

  $query = "select * from `teacher_details` where phone = $_SESSION[teacher_mob]";
