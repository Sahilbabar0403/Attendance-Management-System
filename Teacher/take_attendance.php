...
<html lang="en">
  <head>
    ...
    <title>Lecture Attendance-Page</title>
...
if(isset($_GET['submit_attendance'])=='Submit'){
    ...
    for($i = $s_rollno;$i<=$l_rollno;$i++){
      ...
      echo "</br>".$str_val;
    $date = date("Y-m-d");
    $query = "INSERT INTO `$table_name`( `semester`, `department`, `division`".$str.", `attendance`,`date`) VALUES ('$semester','$department','$division',".$str_val."'submitted','$date' );";

    $result = mysqli_query($conn,$query);
    if($result){
      echo "Data inserted successfully";
      header('location: teacher.php?lect_attendance='.$table_name);
    }
    else{
...
<form class="row g-5 needs-validation" action="" method="get" novalidate>
    ...
    <table class="text-center  table-striped  shadow">
        ...
        <tr  class="table-group-divider">
            <td style="font-weight: bold; font-size: 19px;"> <label style=" z-index: 5; width: 100%; padding: 15px;" for="<?php echo 'ls'.$i ?>"> <?php echo $i ?> </label>
            </td>
            <td><label id="name" style=" z-index: 5; width: 100%; padding: 15px;" for="<?php echo 'ls'.$i ?>"><?php echo "$row_new[name]" ;?></label></td> 
            ...
            <input style="position: absolute; right: 20%; "  class="input"  type="checkbox" onclick="changeColor(this);" value="Present" id="<?php echo 'ls'.$i ?>" name="s<?php echo $i ?>"/>
            <!-- <input style="position: absolute; right: 20%; "    type="checkbox" onclick="changeColor(this);" value="Present" id="<?php echo 'ls'.$i ?>" name="s<?php echo $i ?>"/> -->
            <label   style="position: relative; z-index: 5; width: 100%; padding: 15px; " for="<?php echo 'ls'.$i ?>"> </label>
          </td>
            <!-- <td><input type="radio" onclick="changeColor(this);" value="Present" name="s<?php echo $i ?>"/></td>
            <td><input type="radio" onclick="changeColor1(this);" value="Absent" style="padding: 10px;"  name="s<?php echo $i ?>" checked /></td> -->
        ...
        <?php
            ...
            <div class="col-12">
              <!-- <label for="getCount">Get Count</label>
              <input type="checkbox" name="getCount" id="getCount" id="getCount" onclick="checkboxes()">
              <p id="count"></p> -->
